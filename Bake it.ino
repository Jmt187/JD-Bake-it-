...
// ================= GAME CONFIG =================
#define INITIAL_TIME_LIMIT 3000    // 3 seconds initially
#define MIN_TIME_LIMIT 1000        // Minimum 1 second
#define SPEEDUP_RATE 100           // Reduce time by 100ms each round
...
// Game state
enum GameState {
  STATE_WAITING,      // Waiting for START button
  STATE_INTRO,        // Playing intro sound
...
void handleCorrectInput() {
  ...
  // Speed up the game (reduce time limit)
  if (currentTimeLimit > MIN_TIME_LIMIT) {
    currentTimeLimit -= SPEEDUP_RATE;
  }
...
void loop() {
  ...
  switch(gameState) {
    ...
    case STATE_INTRO:
      ...
      if (millis() - audioDelay > 2000) {  // Wait 2s for intro
        startNewChallenge();
      }
      break;
